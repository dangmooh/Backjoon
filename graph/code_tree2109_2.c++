...
vector<pair<int, int>> findNext(int y, int x, int dir)
{
    ...
    for(const auto& n : next)
    {
        ...
        if(nx != -1 && ny != -1)//경계, 방문처리, 벽인지 확인인
        {
            
            if(canMove(ny,nx,dir))
            {
                nx +=dd[dir].second;
                ny += dd[dir].first;
...
void spreadAir(airConditioner& ac)
{
    ...
    while(!q.empty())
    {
        ...
        for(const pair<int,int>& nextP : s)
        {
            int ny, nx;
            tie(ny,nx) = nextP;
            if(ny!=-1 && nx!=-1 && curP > 1 && !visited[ny][nx]) // 이동 가능하고 현재 바람의 세기가 2이상인 경우
            {
...
int main()
{
    ...
    for(int i=0; i<m; i++)
    {
        int x, y, s;
        cin >> y >> x >> s;
        tmpWall.push_back(make_tuple(y-1, x-1, s));
    }
    findWall();
